...
/*
 * Enhanced crystal color modification system
 */
let CrystalObject;
let crystalUpdateInterval;
const materialInstances = new Set();

// Enhanced function to find CrystalObject with retries
  ...
  try {
    ...
    materialInstances.forEach((material) => {
      try {
        ...
        console.warn('Error updating individual material:', err);
      }
    });
    
    console.log('Updated crystal color:', color);
  } catch (e) {
    console.error('Error in updateCrystalColor:', e);
  }
};

// Initialize crystal color system with retry mechanism
const initCrystalSystem = () => {
  // Clear any existing interval
  if (crystalUpdateInterval) {
    clearInterval(crystalUpdateInterval);
  }

  // Try to find CrystalObject
    ...
    try {
      ...
      CrystalObject.prototype.getModelInstance = function () {
        ...
        try {
          ...
          console.warn('Error in getModelInstance override:', err);
        }

        return instance;
      };

      // Set up periodic check for new materials
      crystalUpdateInterval = setInterval(() => {
        const savedColor = localStorage.getItem('crystal-color');
        if (savedColor) {
          updateCrystalColor(savedColor);
        }
      }, 1000);

      return true;
    } catch (e) {
      console.error('Error setting up crystal system:', e);
      return false;
    }
  }
...
let initialized = initCrystalSystem();

// If not successful, retry with a delay
if (!initialized) {
  console.log('Retrying crystal system initialization in 1 second...');
  setTimeout(() => {
    initialized = initCrystalSystem();
    if (!initialized) {
      console.warn('Failed to initialize crystal system after retry');
    }
  }, 1000);
}

// Set up color picker when DOM is ready
